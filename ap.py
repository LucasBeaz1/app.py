...
def obtener_persona():
    ...
    personas_json = [{'nombre': persona.nombre, 'apellido': persona.apellido, 'email': persona.email, 'dni': persona.dni} for persona in 
    personas]
...
def eliminar_persona(dni):
    ...
    if persona:
        ...
        db.session.commit()
        return jsonify({'mensaje': 'se elimino una persona'}), 200
    return jsonify({'mensaje': 'persona no encontrada'}), 404
